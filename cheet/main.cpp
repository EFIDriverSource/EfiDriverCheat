...
void actorLoop() {
	...
	if (Settings::DrawFOV)
	{
		ImGui::GetOverlayDrawList()->AddCircle(ImVec2(Settings::Majors::ScreenCenterX, Settings::Majors::ScreenCenterY), Settings::AimbotFOVValue, ImGui::ColorConvertFloat4ToU32(ImVec4(255.0, 153.0, 51.0, 255.0)), 10000.f);
	}
		...
		if (TeamIndex != LocalTeam) {
			...
			if (distance <= Settings::EspDistance) {
				...
				if (Settings::ShowDistance) {
					...
					ImGui::GetOverlayDrawList()->AddText(ImVec2(vRootBone.x - 20, vRootBone.y), ImGui::ColorConvertFloat4ToU32(ImVec4(255.0, 153.0, 51.0, 255.0)), dist);
				}
				if (Settings::line)
				{
					ImGui::GetOverlayDrawList()->AddLine(ImVec2(Settings::Majors::ScreenCenterX, Settings::Majors::ScreenCenterY), ImVec2(vHeadBoneOut.x, vHeadBoneOut.y), ImGui::ColorConvertFloat4ToU32(ImVec4(255.0,153.0,51.0,255.0)), 1.f);
				}
...
void renderLoopCall() {
	...
	if (Settings::Majors::menuIsOpen)
	{
		...
		colors[ImGuiCol_WindowBg] = ImVec4(0.f, 0.f, 0.f, 255.00f);
		...
		colors[ImGuiCol_TitleBg] = ImVec4(0.f, 0.f, 0.f, 255.00f);
		colors[ImGuiCol_TitleBgActive] = ImVec4(0.f, 0.f, 0.f, 255.00f);
		colors[ImGuiCol_TitleBgCollapsed] = ImVec4(0.00f, 0.00f, 0.00f, 255.00f);
		...
		colors[ImGuiCol_Separator] = ImVec4(255.f, 255.f, 255.f, 255.f);
		...
		ImGui::SetNextWindowSize({ 450.f,260.f });

		ImGui::Begin("Fortmeme", 0, ImGuiWindowFlags_::ImGuiWindowFlags_NoResize | ImGuiWindowFlags_::ImGuiWindowFlags_NoCollapse);
		ImGui::Separator();
		ImGui::SetCursorPos({ 58.f,42.f });
		if (ImGui::Button("Aimbot", { 90.f,30.f }))
		{
			tabs = 1;
		}
		ImGui::SetCursorPos({ 236.f,42.f });
		if (ImGui::Button("ESP", { 90.f,30.f }))
		{
			tabs = 2;
		}
		if (tabs == 1)
		{
			ImGui::SetCursorPos({ 23.f,95.f });
			ImGui::Checkbox("Softaim", &Settings::Aimbot);
			ImGui::SetCursorPos({ 23.f,127.f });
			ImGui::Checkbox("AimFov", &Settings::DrawFOV);
			ImGui::SetCursorPos({ 23.f,156.f });
			ImGui::PushItemWidth(327.000000);
			ImGui::SliderFloat("AimFOV            ", &Settings::AimbotFOVValue, 0, 200);
			ImGui::PopItemWidth();
			ImGui::SetCursorPos({ 23.f,185.f });
			ImGui::PushItemWidth(327.000000);
			ImGui::SliderFloat("Softaim Smoothness", &Settings::AimbotSmoothingValue, 1, 20);
			ImGui::PopItemWidth();
			ImGui::SetCursorPos({ 23.f,210.f });
		...
		{
			ImGui::SetCursorPos({ 23.f,95.f });
			ImGui::Checkbox("ESP", &Settings::Box);
			ImGui::SetCursorPos({ 23.f,120.f });
			ImGui::Checkbox("Snapline Esp", &Settings::line);
			ImGui::SetCursorPos({ 23.f,145.f });
			ImGui::PushItemWidth(327.000000);
			ImGui::SliderInt("Esp Distance", &Settings::EspDistance, 1, 200);
			ImGui::SetCursorPos({ 23.f,180.f });
			if (ImGui::Button("Close Cheat", { 120.f,35.f }))
			{
